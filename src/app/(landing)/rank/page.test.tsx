...
describe.skip("Rank Page", () => {
    ...
    test("renders the rank page with class options", () => {
        render(
    ...
    test("displays the selected class details when a class is clicked", () => {
        render(
            <Provider store={mockStore}>
                <ComponentWrapper />
            </Provider>,
    ...
    test("shows no data message when there is no answer summary", () => {
        const customStore = configureStore({
            ...
            preloadedState: {
                configuration: {
                    class: [{ id: 1, name: "Warrior", abbreviation: "WR", color: "#000" }],
                },
                classification: {
                    votesByClass: [{ data: 10, color: "#000", label: "Warrior" }],
                    answerSummary: [],
                },
            },
        });
